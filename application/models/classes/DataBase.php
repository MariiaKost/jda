...
class DataBase {
    ...
    public static function getDB($u_host, $u_name, $u_password, $u_db) {
        if (self::$db == null) self::$db = new DataBase($u_host, $u_name, $u_password, $u_db);
        return self::$db;
    }

    /* private-конструктор, подключающийся к базе данных, устанавливающий локаль и кодировку соединения */
    private function __construct($u_host, $u_name, $u_password, $u_db) {
        $this->mysqli = new mysqli($u_host, $u_name, $u_password, $u_db);
